<template>
  ...
  >
    ...
    <el-table :data="pagedData" border stripe size="small" class="w-full">
      ...
      <el-table-column prop="metricValue" label="指标值" width="88" align="center" />
      ...
      <el-table-column prop="expectedLevel" label="预期等级" width="100" align="center">
        <template #default="{ row }">
          <span :class="row.triggered ? 'text-[#F59E0B] font-medium' : 'text-gray-400'">{{
            row.expectedLevel
          }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="expectedEvent" label="预期事件内容" min-width="180" show-overflow-tooltip>
        <template #default="{ row }">
          <span class="text-gray-700">{{ row.expectedEvent }}</span>
        </template>
      </el-table-column>
    </el-table>
...
/** 按时间范围生成模拟日志（每日一条 08:xx，新日期在上） */
function buildMockLogs(metric, rangeStart, rangeEnd) {
  const eventLabel =
    metric?.sector === 'CANTEEN' || (metric?.name && metric.name.includes('晨检'))
      ? '食堂晨检不合格'
      : `${metric?.name || '指标'}超阈值`
  const rows = []
  ...
  for (let t = end.getTime(); t >= start.getTime(); t -= oneDay) {
    ...
    const timeStr = `${y}-${m}-${day} ${pad(hour)}:${pad(minute)}`
    const daySeed = y + Number(m) * 31 + Number(day)
    const triggered = daySeed % 3 !== 0
    const metricValue = triggered ? 1 : 0
    rows.push({
      time: timeStr,
      metricValue,
      triggered,
      expectedLevel: triggered ? '橙色' : '-',
      expectedEvent: triggered ? eventLabel : '-'
    })
  }
  return rows
}
...
function runQuery() {
  if (!props.metric || !dateRange.value?.[0] || !dateRange.value?.[1]) {
    ElMessage.warning('请选择时间范围')
    return
  }
  allLogs.value = buildMockLogs(props.metric, dateRange.value[0], dateRange.value[1])
...
function exportRows() {
  ...
  a.href = url
  a.download = `试运行日志-${props.metric?.code || 'metric'}-${dateRange.value?.join('_') || 'export'}.csv`
  a.click()
